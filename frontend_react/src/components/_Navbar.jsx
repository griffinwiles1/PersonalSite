...
const Navbar = () => {
  ...
  return (
    <nav className="w-full pl-6 flex justify-between items-center bg-footerPrimary navbar navbarShadow z-10">
      ...
      <ul className="list-none sm:flex hidden justify-end items-center flex-1">
        { navLinks.map((nav, index) => (
          <li
            key={ nav.id }
            className={`
              h-[64px] px-6 flex items-center font-poppins font-semibold cursor-pointer text-[24px] text-secondaryWhite
              hover:bg-secondaryDark
              ${ active === nav.title ? "border-b-8 pt-[8px] border-footerSecondary" : "" }
            ...
            { navLinks.map((nav, index) => (
              <li
                ...
                className={` w-[100%] h-[52px] pl-3 flex items-center font-poppins font-semibold text-[20px] text-secondaryWhite 
                  ...
                  ${ active === nav.title ? "border-l-8 border-footerSecondary" : "" }
                  ${ index === 0 ? "mt-3" : "mt-0" }
                `}
                onClick={ () => setActive(nav.title) }
              >
